...
const loadPackageJson = packagePath => {
    try {
        ...
        // If package.json is not found or cannot be read, return null
        return null;
    }
};
...
const getWorkspacesRootConfig = dir => {
    ...
    if (packageObj && Reflect.has(packageObj, 'workspaces')) {
...
const getPackagePaths = (root, workspacesList) => {
    console.log({ root, workspacesList })
    const packageList = [];
    ...
    workspacesList.forEach(workspace => {
        ...
        const packageJsonGlob = path.join(workspaceAbsDir, '**', 'package.json');
        console.log({ packageJsonGlob })
...
const getDeep = (obj, keyChain) => {
    ...
    // Check if obj is an object before using Reflect.has
