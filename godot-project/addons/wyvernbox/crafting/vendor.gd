...
func refill_stock():
	...
	# Clear the inventory by removing all items
	inventory.clear()
	
	# Assuming stock is an array containing ItemStacks
	for i in range(stock.size()):
		var stack = get_stock(i)
		var free_position = inventory.get_free_position(stack)
		if free_position.x >= 0 && free_position.y >= 0:
			_put_up_for_sale(stack, inventory, i)
			inventory.try_place_stackv(stack, free_position)
